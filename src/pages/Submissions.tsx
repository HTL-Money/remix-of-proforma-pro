...
//
// This is the "back end" the recruit never sees. The holdback is derived from
// their actual overhead rather than chosen by them, and this is where the team
// reads it to size an offer. RLS does the real enforcement: anon has no select
// on `proformas`, so only a signed-in team member can load this at all.
    ...
    (async () => {
      try {
        ...
        const { data, error } = await sb
          .from("proformas")
          .select("id, name, source, nmls, annual_volume, lo_split, employee_count, payroll_overhead, derived_holdback_pct, final_lo_net, updated_at")
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-5">
      <div>
        ...
        <p className="text-sm text-white/65 mt-0.5">
          Every pro forma saved or submitted, with derived payroll economics. Internal only — none of
          this is shown to the loan officer.
        </p>
      </div>
        ...
        <section className="glass-panel p-0 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={9} className="py-10 text-center text-white/65"><Loader2 className="h-5 w-5 animate-spin inline" /></td></tr>
                ) : rows.length === 0 ? (
                  <tr><td colSpan={9} className="py-10 text-center text-white/65">No pro formas yet.</td></tr>
                ) : rows.map(r => (
