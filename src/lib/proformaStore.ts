...
export const submitPublicProforma = async (name: string, state: ModelState, recruitEmail?: string, referralToken?: string | null): Promise<void> => {
  const supabase = requireSupabase();
  const { error } = await supabase.from(TABLE).insert({
    ...
    // Which LO's PURL brought this visitor in (null for un-referred visits).
